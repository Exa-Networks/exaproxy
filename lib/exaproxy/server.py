	...
	def start (self):
		try:
			...
			if e.errno == errno.EADDRINUSE:
				logger.error('server','could not listen, connection in use %s:%d' % (self.ip,self.port))
			if e.errno == errno.EADDRNOTAVAIL:
				logger.error('server','could not listen, invalid address %s:%d' % (self.ip,self.port))
			logger.error('server','could not listen on %s:%d - %s' % (self.ip,self.port,str(e)))
			self.close()

	def select (self,read,write):
		#logger.debug('server',"select on read  %s" % str(read))
		#logger.debug('server',"select on write %s" % str(write))
		try:
			r,w,e = select(read,write,read+write,self.speed)
			#logger.debug('server',"ready on read   %s" % str(r))
			#logger.debug('server',"ready on write  %s" % str(w))
			...
			if e.errno in (errno.EAGAIN, errno.EINTR,errno.EWOULDBLOCK,errno.EINTR,): # blocking return
				logger.error('server',"select not ready, errno %s" % str(e.errno))
				return [],[],[]
			if e.errno in (errno.EINVAL,errno.EBADF,): # fatal issues (please do not change this list)
				logger.error('server',"select problem, errno %s" % str(e.errno))
				raise
			logger.error('server',"select issue, debug it, errno %s" % str(e.errno))
...
#					logger.errro('server',"a object without fileno() was passed to select")
#				if obj.fileno() < 0:
#					logger.error('server',"PYPY 1.7 change socket object to return -1 when they are closed, this is an issue for us")
			raise SelectError(str(e))
		except Exception, e:
			logger.error('server',"select problem, %s %s" % (type(e),str(e)))
			raise SelectError(str(e))
	...
	def stop (self):
		logger.info('server',"stop server listening on %s:%s" % (self.ip,str(self.port)))
		self.running = False
		try:
	...
	def run (self):
		if self._server_loop is None:
			...
			logger.error('server','can not run, stopped <server>')
...
#			print "write_opening   ", write_opening 
			...
			if self.io in read:
				for peer in self.accept():
					if peer is None:
						...
						logger.error('server','could not accept new connection, exiting')
						self.running = False
						continue
					else:
						logger.debug('server','new connection, %s' % str(peer))
			...
			for fetcher in set(read_download).intersection(read):
				...
				if fetcher.cid in self.browsers.established_id():
					...
					if webpage:
						logger.debug('server','we fetched %d byte of data for client id %d' % (len(webpage),fetcher.cid))
